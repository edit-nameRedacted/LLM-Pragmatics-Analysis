...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Directory containing locally downloaded model folders. "
            "Overrides the QXC_MODELS_DIR environment variable. "
            "If neither is set, falls back to HuggingFace cache (~/.cache/huggingface/hub)."
        ),
    )
    return p.parse_args()
...
def load_nli_model() -> tuple:
    """Load DeBERTa-base-MNLI on CPU to save GPU VRAM."""
    nli_tok = AutoTokenizer.from_pretrained(NLI_MODEL_ID)
    nli_clf = (
        AutoModelForSequenceClassification.from_pretrained(
            NLI_MODEL_ID, torch_dtype=torch.float32
        )
        .to("cpu")
        .eval()
    )
    print(f"NLI model: {NLI_MODEL_ID} on CPU")
    return nli_tok, nli_clf


def load_sbert_model():
    """Load Qwen3-Embedding-0.6B via SentenceTransformer on CPU for beam similarity.

    Returns None if sentence_transformers is not installed, with a warning.
    CPU-only to preserve GPU VRAM for the main model.
    """
    if not _SBERT_AVAILABLE:
        print(
            "[WARNING] sentence_transformers not installed — "
            "beam_sbert_cosine will be None. Install with: pip install sentence-transformers"
        )
        return None
    sbert = _SentenceTransformer("Qwen/Qwen3-Embedding-0.6B", device="cpu")
    print("SBERT model: Qwen/Qwen3-Embedding-0.6B on CPU")
    return sbert
...
def main() -> None:
    ...
    # Override model directory if --models_dir was supplied
    if args.models_dir is not None:
        models_dir = args.models_dir
        model_cfg = dict(model_cfg)
        model_cfg["path"] = models_dir / model_cfg["path"].name
    ...
    for row in tqdm(pilot_df.itertuples(), total=len(pilot_df), desc="Prompts"):
        ...
        # ── 2. EAS (entropy arc score) — normalised area under entropy curve ──
        # Computed in-memory from token_entropy_sequence; written to
        # extended_metrics.csv only, not to pilot_summary.csv.
        eas_per_sample = [
            float(np.trapezoid(seq) / len(seq)) if seq else 0.0
            for seq in logit_data["token_entropy_sequence"]
        ]
